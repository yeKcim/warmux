...
_Stl_create_filebuf(FILE* f, ios_base::openmode mode ) {
  basic_filebuf<char, char_traits<char> >* result =
    _CHECK_PTR(new basic_filebuf<char, char_traits<char> >());
...
_Stl_create_wfilebuf(FILE* f, ios_base::openmode mode) {
  basic_filebuf<wchar_t, char_traits<wchar_t> >* result =
    _CHECK_PTR(new basic_filebuf<wchar_t, char_traits<wchar_t> >());
...
#  ifdef _STLP_REDIRECT_STDSTREAMS
    ...
    if (_S_was_synced) {
      ptr_cin->init(_CHECK_PTR(new stdio_istreambuf(stdin)));
...
#  else
    ...
    if (_S_was_synced) {
      ptr_cin->init(_CHECK_PTR(new stdio_istreambuf(stdin)));
      ptr_cout->init(_CHECK_PTR(new stdio_ostreambuf(stdout)));
      ptr_cerr->init(_CHECK_PTR(new stdio_ostreambuf(stderr)));
      ptr_clog->init(_CHECK_PTR(new stdio_ostreambuf(stderr)));
    } else {
      ptr_cin->init(_Stl_create_filebuf(stdin, ios_base::in));
      ptr_cin->init(_Stl_create_filebuf(stdout, ios_base::out));
      ptr_cin->init(_Stl_create_filebuf(stderr, ios_base::out));
...
#  endif
  ...
  _STLP_TRY {
    if (sync && !was_synced) {
      new_cin  = _CHECK_PTR(new stdio_istreambuf(stdin));
      new_cout = _CHECK_PTR(new stdio_ostreambuf(stdout));
      new_cerr = _CHECK_PTR(new stdio_ostreambuf(stderr));
      new_clog = _CHECK_PTR(new stdio_ostreambuf(stderr));
    }
