/*
 * libxml2_xinclude.c : Code to implement XInclude processing
 *
 * World Wide Web Consortium W3C Last Call WorkingDraft 10 November 2003
 ...
 */
static int
xmlXIncludeURLPush(xmlXIncludeCtxtPtr ctxt, const xmlChar* value)
{
    xmlChar* vcopy;
    ...
    if (ctxt->urlTab == NULL) {
        ...
        ctxt->urlTab = (xmlChar**) xmlMalloc(4 * sizeof(ctxt->urlTab[0]));
        if (!ctxt->urlTab)
            goto OOM_exit;
        ctxt->urlMax = 4;
    }
    if (ctxt->urlNr >= ctxt->urlMax) {
        xmlChar** tmp;
        ctxt->urlMax *= 2;
        tmp =  (xmlChar**) xmlRealloc(ctxt->urlTab, // DONE: Fix xmlRealloc
                                      ctxt->urlMax *
...
OOM_exit:
    ...
    vcopy = value ? xmlStrdup(value) : (xmlChar*) value /* NULL */;
    if(vcopy || !value)
        {
        ctxt->url = ctxt->urlTab[ctxt->urlNr] = vcopy;
        return (ctxt->urlNr++);
        }
    else // OOM!
        goto OOM_exit;
...
xmlXIncludeLoadTxt(xmlXIncludeCtxtPtr ctxt, const xmlChar *url, int nr) {
    ...
    if (encoding != NULL) {
        /*
         *  we should not have to remap to the xmlCharEncoding
    ...
    for (i = 0;i < len;) {
        int cur;
        int k; // DONE: Rename  'l' --> 'k'

        cur = xmlStringCurrentChar(NULL, &content[i], &k);
        if (!IS_CHAR(cur)) {
        xmlXIncludeErr(ctxt, ctxt->incTab[nr]->ref,
                       XML_XINCLUDE_INVALID_CHAR,
                   "%s contains invalid char\n", URL);
        } else {
        xmlNodeAddContentLen(node, &content[i], k); // may set OOM flag
        }
        i += k;
    }
