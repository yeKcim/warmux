/*
 * libxml2_htmlparser.c : an HTML 4.0 non-verifying parser
 *
...
static const int htmlOmittedDefaultValue = 1;
 ...
 */
void
htmlErrMemory(xmlParserCtxtPtr ctxt, const char *extra); // moved to XSLT-enabled part of this file
...
htmlnamePush(htmlParserCtxtPtr ctxt, const xmlChar * value)
{
    if (ctxt->nameNr >= ctxt->nameMax) {
        void* allocTmp; // DONE: Fix xmlRealloc
        allocTmp = xmlRealloc((xmlChar**)ctxt->nameTab,
        ...
        ctxt->nameMax *= 2;
        ctxt->nameTab = (const xmlChar**) allocTmp;
    }
...
// TO DO libxslt added 2nd const in between
...
#endif /* defined(LIBXML_HTML_ENABLED) || defined(XMLENGINE_XSLT) */
...
htmlInitAutoClose(void) {
    ...
    while ((htmlStartClose[i] != NULL) && (indx < 100 - 1)) {
        // libxslt port: (const char**) cast was added
 ...
 * @param ctxt an HTML parser context
 * @param newtag The new tag name
 * @param force force the tag closure
 *
...
/*
 * Macro used to grow the current buffer. Buffer is freed in OOM.
 */ // DONE: Fix xmlRealloc  
#define growBuffer(buffer) {                                            \
    void* allocTmp;                                                     \
    buffer##_size *= 2;                                                 \
    allocTmp = xmlRealloc(buffer, buffer##_size * sizeof(xmlChar));     \
    if (!allocTmp) {                                                    \
        xmlFree(buffer);                                                \
        htmlErrMemory(ctxt, "growing buffer\n");                        \
        return(NULL);                                                   \
    }                                                                   \
    buffer = (xmlChar*) allocTmp;                                       \
}
 ...
 */
XMLPUBFUNEXPORT const htmlElemDesc *
...
htmlParseComment(htmlParserCtxtPtr ctxt)
{
    ...
    if (!buf)
        goto OOM_exit;
    // Now we must free 'buf' before returning
    q = CUR_CHAR(ql);
    NEXTL(ql);
    r = CUR_CHAR(rl);
    NEXTL(rl);
...
OOM_exit:
    ...
    } // end of "while good character and not the end of comment (-->)"
...
htmlInitParserCtxt(htmlParserCtxtPtr ctxt)
{
    ...
    // NOTE: All assignments  ctxt->XX = 0; were commented as unnecessary
 ...
 * @param comment flag to force checking inside comments
 *
 ...
 * @param enc an optional encoding
 *
...
htmlReadDoc(const xmlChar * cur, const char *URL, const char *encoding, int options)
{
    ...
    ctxt = xmlCreateDocParserCtxt(cur, sizeof(cur));
    if (ctxt == NULL)
        return (NULL);
