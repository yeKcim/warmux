/*
 * libxml2_xmlstring.c : an XML string utilities module
 *
...
xmlStrndup(const xmlChar *cur, int len) {
    ...
    if (!ret) {
        // NOTE: error reporting is removed; it will be done from xmlMalloc	
        return ret; // ret is NULL here
    }
...
xmlStrdup(const xmlChar *cur) {
    ...
    if (!cur)
        { // This should not normally happen -- check argument before calling!
        // NOTE: Avoiding usage of NULL here improves generated assembly!
        return(xmlChar*)cur /* NULL */;
        }
    while (*p != 0)
