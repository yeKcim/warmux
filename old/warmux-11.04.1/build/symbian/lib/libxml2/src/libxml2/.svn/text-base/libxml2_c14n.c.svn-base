...
/**
 * xmlC14NNsCompare:
 * @param ns1 the pointer to first namespace
 * @param ns2 the pointer to second namespace
 *
...
/**
 * xmlC14NAttrsCompare:
 * @param attr1 the pointer tls o first attr
 * @param attr2 the pointer to second attr
 *
...
/**
 * xmlC14NPrintAttrs:
 * @param attr the pointer to attr
 * @param ctx the C14N context
 *
...
xmlC14NProcessAttrsAxis(xmlC14NCtxPtr ctx, xmlNodePtr cur)
{
        ...
        !xmlC14NIsVisible(ctx, cur->parent, cur->parent->parent))
    {
        ...
        cur = cur->parent;
// XML Engine: NEW CODE
        // NOTE: We must check that 'cur' is not a document node,
        //       otherwise xmlDocPtr is wrongly treated as xmlNodePtr
        //       (node-specific fields are accessed)
        // ORIGINAL: while (cur != NULL)
        while(cur && cur->type != XML_DOCUMENT_NODE)
//
        {
            attr = cur->properties; // NOTE: We may still have problem here if *cur is xmlEntity
            while (attr)
            {
                if (attr->ns &&
...
xmlC14NCheckForRelativeNamespaces(xmlC14NCtxPtr ctx, xmlNodePtr cur)
{
    ...
    while (ns) {
        if (xmlStrlen(ns->href) > 0) {
            ...
            if (!xmlStrEqual((const xmlChar*) uri->scheme, BAD_CAST "urn") &&
...
xmlC14NProcessElementNode(xmlC14NCtxPtr ctx, xmlNodePtr cur, int visible)
{
    ...
    if (visible) {
        ...
        if (cur->ns && xmlStrlen(cur->ns->prefix) > 0) {
 ...
 *
 * OOM: possible --> NULL is returned for valid arguments
 */
 ...
 * @param user_data the first parameter for is_visible_callback function
 *                      (in most cases, it is nodes set)
 * @param exclusive the exclusive flag (0 - non-exclusive canonicalization;
 ...
 * @param compression the compression level (zlib requred):
 *                              -1 - libxml default,
 *                               0 - uncompressed,
 *                              >0 - compression level
 *
...
#define growBufferReentrant() {                                 \
    void* tmp;                                                  \
    buffer_size *= 2;                                           \
    tmp = xmlRealloc(buffer, buffer_size * sizeof(xmlChar));    \
    if (!tmp) {                                                 \
        /* Note: must free the buffer */                        \
        xmlFree(buffer);                                        \
