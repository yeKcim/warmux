/*
 * libxml2_valid.c : part of the code use to do the DTD handling and the validity
 *                   checking
 *
 ...
 * @param error the error number
 * @param str1 extra informations
 * @param str2 extra informations
 * @param str3 extra informations
 *
...
xmlNewElementContent(const xmlChar* name, xmlElementContentType type)
{
    ...
    /* Unneded code: these fields are NULL after memset()
     else {
        ret->name = NULL;
        ret->prefix = NULL;
    }
 ...
 * @param content An element table
 * @param glob 1 if one must print the enclosing parenthesis, 0 otherwise
 *
 ...
 * @param content An element table
 * @param glob 1 if one must print the enclobing parenthesis, 0 otherwise
 *
 ...
 */
XMLPUBFUNEXPORT xmlElementTablePtr
 ...
 */
XMLPUBFUNEXPORT xmlAttributePtr
 ...
 */
XMLPUBFUNEXPORT xmlAttributeTablePtr
 ...
 * Returns NULL if not, otherwise the entity
 */
XMLPUBFUNEXPORT xmlNotationPtr
 ...
 */
XMLPUBFUNEXPORT xmlNotationTablePtr
...
xmlAddID(xmlValidCtxtPtr ctxt, xmlDocPtr doc, const xmlChar* value, xmlAttrPtr attr)
{
    ...
    if(!ret->value)
        goto OOM; // note: value is not NULL for sure!
    ...
    if (ctxt && (ctxt->vstateNr != 0))
    {
        ...
        else
        {
            ...
            if(!ret->name)
                goto OOM; // attr->name may not be NULL
        }
        ret->attr = NULL;
    } else {
        ret->attr = attr;
...
OOM:
    ...
    // DONE: removed redundant check: // if (attr)
...
xmlIsID(xmlDocPtr doc, xmlNodePtr elem, xmlAttrPtr attr)
{
    ...
    // DONE: Optimized: if elem is NULL it always returs 0 (in all branches)
    if (!doc || !attr || !elem ||
    ...
    if (doc->type == XML_HTML_DOCUMENT_NODE)
    {
        if ((xmlStrEqual(BAD_CAST "id", attr->name)    ||
             xmlStrEqual(BAD_CAST "name", attr->name))
               &&
             !xmlStrEqual(elem->name, BAD_CAST "input"))
        {
            return(1);
        }
        return(0);
    ...
    {   // we have XML document
        xmlAttributePtr attrDecl;

        //DONE: 'elem' is check before; REMOVED: if (!elem) return(0);
...
// forward declaration
int xmlAddIDsInternal(xmlDocPtr doc, xmlNodePtr cur, const xmlChar** ids);

/**
 * xmlAddIDs:
 * @param root:     the pointer to an XML node.
 * @param ids:      the pointer to a NULL terminated list of pairs {name, uri}
                    for looked up ID attributes.
 * @return result code:
                0 if OK;
                -1 if duplication of ID happend;
                -2 if OOM;
                -3 if invalid arguments
 *
 * Recursively walks through all children of the root node and adds all attributes
 * from the ids list to the root's document's IDs attributes hash.
 *
 * 
 *       
 *
 * Prerequisites:  TRUE == root && root->docs
 * OOM: possible
 */
XMLPUBFUNEXPORT int 
xmlAddIDs(xmlNodePtr root, const xmlChar** ids)
{
    int ret;
    LOAD_GS_SAFE_NODE(root)

    if(!root || !root->doc)
        return -3;

    ret = xmlAddIDsInternal(root->doc, root, ids);
    if(OOM_FLAG)
        ret = -2;
...
int 
xmlAddIDsInternal(xmlDocPtr doc, xmlNodePtr cur, const xmlChar** ids)
{
    int ret = 0;
    ...
    if(cur && cur->type == XML_ELEMENT_NODE)
    {
        int i;
        xmlAttrPtr attr;
        const xmlChar* idAttrName;
        //const xmlChar* idAttrUri;
        
        for(attr = cur->properties; attr; attr = attr->next)
        {
            for(i = 0; (idAttrName = ids[i]); i += 2)
            {
                if (xmlStrEqual(attr->name, idAttrName) &&
                   ((attr->ns && xmlStrEqual(attr->ns->href,ids[i+1])) || (!attr->ns && !ids[i+1])))
                {
                    // Matching attribute found, get value (into 'name')
                    // and create ID info in the document
                    xmlChar* name = xmlNodeListGetString(doc, attr->children, 1);
                    if(name)
                    {
                        xmlAttrPtr tmp = xmlGetID(doc, name);
                        if(!tmp)
                        {  // No such ID yet, try to add it
                           if(!xmlAddID(NULL, doc, name, attr))
                           {
                               ret = -2; // OOM
                           }
                        }
    ...
    while(children) {
        if(children->type == XML_ELEMENT_NODE) {
            ret = xmlAddIDsInternal(doc, children, ids);
            if(ret != 0)
               return ret;
        }
...
/**
 * xmlFreeRefList:
 * @param list_ref A list of references.
 *
 ...
 * @param dtd a pointer to the DtD to search
 * @param name the element name
 * @param create create an empty description if not found
 *
 * Search the DTD for the description of this element
 *
 * returns the xmlElementPtr if found or NULL
...
xmlGetDtdAttrDesc(xmlDtdPtr dtd, const xmlChar *elem, const xmlChar *name)
{
    xmlAttributeTablePtr table;
    xmlAttributePtr cur;
    xmlChar* uqname; // DONE: removed unneeded initialization // = NULL;
    xmlChar* prefix; // = NULL; // xmlSplitQName2 sets it NULL as the first thing
    LOAD_GS_SAFE_DTD(dtd)
 ...
 * @param name the attribute name
 * @param value the attribute value
 * @param ctxt the validation context or NULL
 *
 ...
 * @param list an array to store the list of child names
