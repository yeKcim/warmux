...
#  endif
#  if (defined(__BEOS__) && defined(__INTEL__)) || defined(__PIPS__)
#    include <fcntl.h>
#    include <sys/stat.h>         // For _fstat
...
#  endif
#elif defined(_STLP_USE_SYMBIAN_IO)
#  include <f32file.h>
#else
#  error "Configure i/o !"
#endif
...
#elif defined (_STLP_USE_SYMBIAN_IO)
const _STLP_fd INVALID_STLP_FD = RFile();
#else
#  error "Configure i/o !"
#endif
...
// Symbian file server handle
#if defined (_STLP_USE_SYMBIAN_IO)
static RFs __sym_fs;
static int __sym_fs_count = 0;
#endif
...
#if !defined(__MSL__) && !defined(__MRC__) && !defined(__SC__) && !defined(_STLP_WCE) && !defined(_STLP_USE_SYMBIAN_IO)    //*TY 04/15/2000 - exclude mpw compilers also
...
#  pragma unused(fd)
  ...
  TUint att;
  if (fd.Att(att) != KErrNone)
    return false;
  return (att & (KEntryAttDir | KEntryAttVolume)) == 0;
...
{
#if defined (_STLP_USE_SYMBIAN_IO)
	if (User::LockedInc(__sym_fs_count) == 0) {
		if (__sym_fs.Connect() != KErrNone)
			return;
		...
		if (parser.SetNoWild(RProcess().FileName(), 0, 0) == KErrNone) {
			...
			if (__sym_fs.CreatePrivatePath(drive) != KErrNone) {
				drive = EDriveC;
				__sym_fs.CreatePrivatePath(drive);
			}
			
			__sym_fs.SetSessionToPrivate(drive);
		}
	}
#endif
}
...
{
#if defined (_STLP_USE_SYMBIAN_IO)
	if (User::LockedDec(__sym_fs_count) == 1)
		__sym_fs.Close();
#endif
}
...
bool _Filebuf_base::_M_open(const char* name, ios_base::openmode openmode,
  ...
  TBuf<256> sname;
  sname.Copy(TPtrC8((unsigned char*)name));

  TInt filemode = EFileStream | EFileShareAny;
  ...
  if ((openmode & ios_base::trunc) || !(openmode & ios_base::in)) {
	if (file_no.Replace(__sym_fs, sname, filemode) != KErrNone)
		return false;
  }
  else
	if (file_no.Open(__sym_fs, sname, filemode) != KErrNone)
	  return false;
	
  if (openmode & ios_base::ate ) {
    TInt ofs = 0;
    if (file_no.Seek(ESeekEnd, ofs) != KErrNone) {
	  file_no.Close();
	  return false;
	}
  }
...
#elif defined (_STLP_USE_UNIX_IO) || defined (_STLP_USE_UNIX_EMULATION_IO)
  int flags = 0;
...
ptrdiff_t _Filebuf_base::_M_read(char* buf, ptrdiff_t n) {
  ...
  TPtr8 pbuf((unsigned char*)buf, 0, n);
  if (_M_file_id.Read(pbuf, n) != KErrNone)
    return -1;
  return pbuf.Length();
...
bool _Filebuf_base::_M_write(char* buf, ptrdiff_t n) {
  for (;;) {
	...
	if (_M_openmode & ios_base::app) {
		TInt ofs = 0;
		if (_M_file_id.Seek(ESeekEnd, ofs) != KErrNone)
			return false;
	}
	TPtrC8 pbuf((unsigned char*)buf, n);
	if (_M_file_id.Write(pbuf) != KErrNone)
		return false;
	written = pbuf.Length();
...
#elif defined(_STLP_USE_SYMBIAN_IO)
#  define STL_SEEK_SET ESeekStart
#  define STL_SEEK_CUR ESeekCurrent
#  define STL_SEEK_END ESeekEnd
#else
...
streamoff _Filebuf_base::_M_seek(streamoff offset, ios_base::seekdir dir) {
  ...
  TInt ofs = (TInt)offset;
  if (_M_file_id.Seek((TSeek)whence, ofs) != KErrNone)
	return streamoff(-1);
  return streamoff(ofs);
