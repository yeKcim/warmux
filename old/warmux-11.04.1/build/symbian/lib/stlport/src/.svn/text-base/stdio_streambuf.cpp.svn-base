...
// cannot use WIN32_IO implementation since missing stat
	...
	// when using Symbian IO, the __file_size() function accepts a RFile
	// handle rather than a unix-style file descriptor, so we can't use it here.
	struct stat sb;
	if (fstat(fd, &sb) < 0)
		return -1;
	streamoff size = sb.st_size;
#else
