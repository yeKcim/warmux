/*
 * libxml2_xmlschemastypes.c : implementation of the XML Schema Datatypes
...
xmlSchemaInitTypes(void)
{
    ...
    xmlSchemaTypesBank = xmlHashCreate(40); // number of simple types
 ...
 */
XMLPUBFUNEXPORT xmlSchemaTypePtr
...
#define RETURN_TYPE_IF_VALID(t)                 \
    ...
    if ((cur[0] == '-') && (cur[1] == '-')) {
    /*
     * It's an  date (xs:gMonthDay, xs:gMonth or
     * xs:gDay)
     */
    cur += 2;
 ...
 * @param llo pointer to the low result
 * @param lmi pointer to the mid result
 * @param lhi pointer to the high result
 *
...
xmlSchemaCompareDates (xmlSchemaValPtr x, xmlSchemaValPtr y)
{
    ...
    if (x->value.date.tz_flag) {
        ...
        if (!y->value.date.tz_flag) {
            ...
            //  OOM: Check p1!=NULL and then OOM flag if p1==NULL
