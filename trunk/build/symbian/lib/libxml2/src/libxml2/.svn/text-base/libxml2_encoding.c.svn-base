/*
 * libxml2_encoding.c : implements the encoding conversion functions needed for XML
 *
 * Related specs:
 ...
 * @param in a pointer to an array of ASCII chars
 ...
 * @param out a pointer to an array of bytes to store the result
 * @param outlen the length of out
 * @param inb a pointer to an array of UTF-8 chars
 * @param inlenb the length of in in UTF-8 chars
 *
 ...
 * @param out a pointer to an array of bytes to store the result
 * @param outlen the length of out
 * @param inb a pointer to an array of UTF-16LE passwd as a byte array
 * @param inlenb the length of in in UTF-16LE chars
 *
...
/**
 * UTF8ToUTF16LE:
 * @param outb a pointer to an array of bytes to store the result
 * @param outlen the length of outb
 * @param in a pointer to an array of UTF-8 chars
 * @param inlen the length of in
 *
 ...
 * @param out a pointer to an array of bytes to store the result
 * @param outlen the length of out
 * @param inb a pointer to an array of UTF-16 passed as a byte array
 * @param inlenb the length of in in UTF-16 chars
 *
 ...
 * OOM: never
 */
XMLPUBFUNEXPORT xmlCharEncoding
...
xmlAddEncodingAlias(const char* name, const char* alias)
{
    ...
    if (xmlCharEncodingAliases == NULL) {
        ...
        tmp = xmlRealloc(xmlCharEncodingAliases,
                         xmlCharEncodingAliasesMax * sizeof(xmlCharEncodingAlias));
        if(!tmp){
            xmlCharEncodingAliasesMax /= 2;
            return -1;
        }
        xmlCharEncodingAliases = (xmlCharEncodingAliasPtr) tmp;
    }
    /*
     * Walk down the list looking for a definition of the alias
     */
 ...
 */
XMLPUBFUNEXPORT xmlCharEncodingHandlerPtr
xmlNewCharEncodingHandler(const char *name,
     ...
     */
    if (name == NULL) {
        xmlGenericError(xmlGenericErrorContext, EMBED_ERRTXT("xmlNewCharEncodingHandler : no name !\n"));
        return(NULL);
    }
    for (i = 0;i < 499;i++) {
        upper[i] = toupper(name[i]);
    ...
    if (up == NULL) {
        xmlGenericError(xmlGenericErrorContext, EMBED_ERRTXT("xmlNewCharEncodingHandler : out of memory !\n"));
        return(NULL);
    }
...
#ifdef DEBUG_ENCODING
    xmlGenericError(xmlGenericErrorContext, "Registered encoding handler for %s\n", name);
#endif
    return(handler);
}
...
xmlInitCharEncodingHandlers(void) {
    ...
    if (handlers == NULL) {
        xmlGenericError(xmlGenericErrorContext, EMBED_ERRTXT("xmlInitCharEncodingHandlers : out of memory !\n"));
        return;
    }
    ...
    if (*ptr == 0x12)
        ...
        xmlGenericError(xmlGenericErrorContext, EMBED_ERRTXT("Odd problem at endianness detection\n"));
...
xmlRegisterCharEncodingHandler(xmlCharEncodingHandlerPtr handler) {
    ...
    if (handler == NULL) {
        xmlGenericError(xmlGenericErrorContext, EMBED_ERRTXT("xmlRegisterCharEncodingHandler: NULL handler !\n"));
        return;
    }

    if (nbCharEncodingHandler >= MAX_ENCODING_HANDLERS) {
        xmlGenericError(xmlGenericErrorContext, EMBED_ERRTXT("xmlRegisterCharEncodingHandler: Too many handler registered\n"));
...
xmlFindCharEncodingHandler(const char *name) {
    ...
    char upper[100];
    int i;
    // 'handlers' is global variable:  
    if (handlers == NULL){
        xmlInitCharEncodingHandlers();
        if(OOM_FLAG)
            return NULL;
    }
    // NULL and "" encoding name means 'default one'
    if (!name || !*name) return(xmlDefaultCharEncodingHandler);
...
#ifdef DEBUG_ENCODING
            xmlGenericError(xmlGenericErrorContext, "Found registered handler for encoding %s\n", name);
#endif
            return(handlers[i]);
        }
...
#ifdef DEBUG_ENCODING
            xmlGenericError(xmlGenericErrorContext, "Found iconv handler for encoding %s\n", name);
#endif
        return enc;
    } else if ((icv_in != (iconv_t) -1) || icv_out != (iconv_t) -1) {
        xmlGenericError(xmlGenericErrorContext, "iconv : problems with filters for '%s'\n", name);
    }
#endif /* LIBXML_ICONV_ENABLED */
...
/**
 * xmlIconvWrapper:
 * @param cd iconv converter data structure
 * @param out a pointer to an array of bytes to store the result
 * @param outlen the length of out
 * @param in a pointer to an array of ISO Latin 1 chars
 * @param inlen the length of in
...
/**
 * xmlCharEncFirstLine:
 * @param handler char enconding transformation data structure
...
xmlCharEncFirstLine(xmlCharEncodingHandler *handler, xmlBufferPtr out,
    ...
    toconv = in->use;
    if (toconv * 2 >= written) {
        xmlBufferGrow(out, toconv); // may set OOM flag
                 ...
                 xmlBufferPtr in)
{
    ...
    written = out->size - out->use;
    if (toconv * 2 >= written) {
        xmlBufferGrow(out, out->size + toconv * 2); // may set OOM flag
        written = out->size - out->use - 1;
    }
    if (handler->input != NULL) {
    ...
    switch (ret) {
        ...
        case -2:
            xmlGenericError(xmlGenericErrorContext,
                            EMBED_ERRTXT("input conversion failed due to input error\n"));
            xmlGenericError(xmlGenericErrorContext,
 ...
 *        the result of transformation can't fit into the encoding we want), or
 *
 * OOM: unknown status / function pointers are in use
 *      possible, but reporting is not guaranteed 
 */
XMLPUBFUNEXPORT int
...
retry:
    ...
    if (ret == -1) {
        ...
        xmlGenericError(xmlGenericErrorContext, EMBED_ERRTXT("xmlCharEncOutFunc: no output function !\n"));
        return(-1);
    }
    ...
    switch (ret) {
        case 0:
#ifdef DEBUG_ENCODING
        xmlGenericError(xmlGenericErrorContext, "converted %d bytes to %d bytes of output\n", toconv, written);
#endif
        break;
        case -1:
#ifdef DEBUG_ENCODING
...
#ifdef DEBUG_ENCODING
        xmlGenericError(xmlGenericErrorContext, "handling output conversion error\n");
        ...
        xmlGenericError(xmlGenericErrorContext,
            EMBED_ERRTXT("output conversion failed due to conv error\n"));
        xmlGenericError(xmlGenericErrorContext,
            EMBED_ERRTXT("Bytes: 0x%02X 0x%02X 0x%02X 0x%02X\n"),
            in->content[0], in->content[1],
            in->content[2], in->content[3]);
...
#ifdef DEBUG_ENCODING
    if (ret)
        xmlGenericError(xmlGenericErrorContext, "failed to close the encoding handler\n");
    else
...
xmlByteConsumed(xmlParserCtxtPtr ctxt) {
    ...
    if (in->end - in->cur > 0) {
        ...
        unsigned char *convbuf = (unsigned char *)malloc(32000);
 ...
 * @param in a pointer to an array of UTF-8 chars
 * @param inlen the length of in
 * @param xlattable the 2-level transcoding table
 *
