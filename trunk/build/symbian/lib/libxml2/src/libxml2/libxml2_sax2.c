/*
 * libxml2_sax2.c : Default SAX2 handler to build a tree.
 *
           ...
           const xmlChar *ExternalID, const xmlChar *SystemID)
{
    ...
    if (ctxt->inputTab == NULL) {
        ...
        ctxt->sax->error(ctxt->userData,
             EMBED_ERRTXT("xmlSAX2ExternalSubset: out of memory\n"));
        ctxt->errNo = XML_ERR_NO_MEMORY;
        ctxt->instate = XML_PARSER_EOF;
        ctxt->disableSAX = 1;
        ctxt->input = oldinput;
          ...
          xmlEnumerationPtr tree)
{
    ...
    else
    {
        if ((ctxt->sax != NULL) && (ctxt->sax->error != NULL))
            ctxt->sax->error(ctxt->userData,
                EMBED_ERRTXT("SAX.xmlSAX2AttributeDecl(%s) called while not in subset\n"), name);
           ...
           const xmlChar *notationName)
{
    ...
    {
        ctxt->sax->error(ctxt->userData,
                EMBED_ERRTXT("SAX.xmlSAX2UnparsedEntityDecl(%s) called while not in subset\n"), name);
    }
}
 ...
 */
XMLPUBFUNEXPORT void xmlSAX2StartDocument(void *ctx)
{
	xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr) ctx;
    xmlDocPtr doc = NULL;
    ...
    if (ctxt->html) {
        ...
        if (!ctxt->myDoc)
            goto OOM;
#else
        xmlGenericError(xmlGenericErrorContext, EMBED_ERRTXT("libxml2 built without HTML support\n"));
...
OOM:
    if ((ctxt->sax != NULL) && (ctxt->sax->error != NULL))
        ctxt->sax->error(ctxt->userData, EMBED_ERRTXT("SAX.xmlSAX2StartDocument(): out of memory\n"));
    ctxt->errNo = XML_ERR_NO_MEMORY;
ERR:
    ctxt->instate = XML_PARSER_EOF;
    ctxt->disableSAX = 1;
        ...
        ctxt->myDoc && ctxt->myDoc->intSubset)
    {
        ...
        if (!ctxt->replaceEntities) {
            ...
            if (val == NULL)
                 ...
                 */
                nvalnorm = xmlValidNormalizeAttributeValue(ctxt->myDoc, ctxt->node, fullname, val);
                if (nvalnorm != NULL) {
                    xmlFree(val);
                    val = nvalnorm;
                }
...
{
    xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr) ctx;
#if defined(LIBXML_VALID_ENABLED) || defined(XMLENGINE_ENABLE_PARSER_RECORD_INFO)
    xmlNodePtr cur = ctxt->node;
#endif
